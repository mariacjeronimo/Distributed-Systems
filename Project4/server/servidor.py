...
def requestArtista(id_artista): #READ ARTISTA
	...
	resp = spotify.get(url).json()
	if "error" in resp:
		if resp["error"]["status"] == 401:
			return resp #TOKEN JÁ EXPIROU

		return None #O ID NAO EXISTE LOL
	else:
		return resp
...
client_id = 'f01f8ed7891a438ebfdc5c00cd99a925'
client_secret = '175473b9c01b4007a96d9f9f6e7ce4da'
...
@app.route('/callback', methods = ["GET"]) #nao é suposto entrar aqui lol
def callback():
...
def utilizadores(id = None): 
	if request.method == "GET":
		if request.url == 'https://localhost:5000/utilizadores/' + str(id): #READ UTILIZADOR <id_user> 
			try:
				...
				if linha is not None and len(linha) != 0:
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores/'+str(id),'httpStatus': 404, 'title' : 'Utilizador nao encontrado'},indent=4) 
					status = 404
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores/'+str(id),'httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r

		if request.url == 'https://localhost:5000/utilizadores/all/':#READ ALL UTILIZADORES 
			try:
				...
				if linha is not None and len(linha) != 0:
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores/all/','httpStatus': 404, 'title': 'Nao existem utilizadores atualmente!'},indent=4)
					status = 404
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores/all/','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
	if request.method == "PUT":
		if request.url == 'https://localhost:5000/utilizadores': #CREATE UTILIZADOR <nome> <senha> 
			...
			try:
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
		...
		if request.url == 'https://localhost:5000/utilizadores/'+ str(id): #UPDATE UTILIZADOR <id_user> <password>
			dados = json.loads(request.data)
		...
		if request.url == 'https://localhost:5000/utilizadores/playlist': #<id_user><id_musica><avaliacao> 
			dados = json.loads(request.data)
			...
			try:
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores/playlist','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/utilizadores/playlist','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
		...
		if request.url == 'https://localhost:5000/utilizadores/playlist/' + str(id): # UPDATE MUSICA <id_musica> <avaliacao> <id_user> 
			dados = json.loads(request.data)
	...
	if request.method == "DELETE":
		if request.url == 'https://localhost:5000/utilizadores/' + str(id): #DELETE UTILIZADOR <id_user> 
			try:
				g.db[1].execute("DELETE FROM utilizadores WHERE id=?", (int(id),))
				g.db[0].commit()
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
			...
			r = make_response(resposta, status, {'Content-Type': 'application/json'})
			return r
		if request.url == 'https://localhost:5000/utilizadores/all/':#DELETE ALL UTILIZADORES 
			try:
...
def musicas(id = None):
	if request.method == "GET": 
		if request.url == 'https://localhost:5000/musicas/' + str(id): #READ MUSICA <id_musica> ------ usa SPOTIFY API
			dados=json.loads(request.data)
			...
			try:
				...
				if cur_id_spotify_fetch is None:
					...
					json_spotify = requestMusica(str(id_spotify))
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/'+str(id),'httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r

		if request.url == 'https://localhost:5000/musicas/all/': #READ ALL MUSICAS
			try:
				cur = g.db[1].execute("SELECT * FROM musicas")
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
				linha = cur.fetchall() 
				if linha is not None and len(linha) != 0:
					...
					for info in linha:
						...
						nome = info[2]
						id_artista= info[3]
						resposta_musica  = requestMusica(str(id_spotify))
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/','httpStatus': 404, 'title' :'Nao existem musicas atualmente!'},indent=4)
					status = 404
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r

		if request.url == 'https://localhost:5000/musicas/all/artista/' + str(id): #READ ALL MUSICAS_A <id_artista> 
			dados=json.loads(request.data)
			...
			try:
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/artista/'+str(id),'httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
				...
				if linha is not None and len(linha) != 0:
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/artista/'+str(id),'httpStatus': 404, 'title' :'Nao existem musicas avaliadas do artista de id= '+ str(id)},indent=4)
					status = 404
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r

		if request.url == 'https://localhost:5000/musicas/all/utilizador/' + str(id): #READ ALL MUSICAS_U <id_user> 
			dados=json.loads(request.data)
			...
			try:
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/utilizador/'+str(id),'httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
				if linha is not None and len(linha) != 0:
					corresp_avaliacao={1:'M', 2:'m', 3:'S',4:'B', 5:'MB'}
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/utilizador/'+str(id),'httpStatus': 404, 'title' :'Nao existem musicas avaliadas do utilizador de id= '+ str(id)},indent=4)
					status = 404

			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r

		if request.url == 'https://localhost:5000/musicas/all/avaliacao': #READ ALL MUSICAS <avaliacao> 
			dados = json.loads(request.data)
			...
			try:
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/avaliacao','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
				if linha is not None and len(linha) != 0:
					corresp_avaliacao={1:'M', 2:'m', 3:'S',4:'B', 5:'MB'}
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas/all/avaliacao','httpStatus': 404, 'title' : 'Nao existem musicas com a avaliacao ='+ str(dados['avaliacao'])},indent=4)
					status = 404
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
	...
	if request.method == "PUT":
		if request.url == 'https://localhost:5000/musicas': #CREATE MUSICA <id_spotify>  
			dados = json.loads(request.data)
			resposta_musica  = requestMusica(str(dados['id_spotify']))
			if resposta_musica is None:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas','httpStatus': 404, 'title' : 'O id do spotify eh invalido'},indent=4)
				status = 404
				r = make_response((resposta, status, {'Content-Type': 'application/json'}))
				return r
			if "error" not in resposta_musica:
				nome_musica =getNomeFromMusica(resposta_musica)
				id_artista = getIdArtistaFromMusica(resposta_musica)
				...
				if existe_musica is None:
					if id_artista_bd is None:
						resposta_artista = requestArtista(id_artista)
						nome_artista = getNomeFromArtista(resposta_artista)
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas','httpStatus': 409, 'title' : "Ja existe uma musica com nome= " + str(nome_musica) +  " e id= " + str(dados['id_spotify']) + " inserida na base de dados"},indent=4)
					status = 409
			else:
				resposta = resposta_musica
				status = 401
	...
	if request.method == "DELETE":
		if request.url == 'https://localhost:5000/musicas/' + str(id): #DELETE MUSICA <id_musica> 
			try:
			...
			if cur_existe is not None:
				...
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/musicas','httpStatus': 404, 'title' : 'Nao existe nenhuma musica de ID= ' + str(id)+' na base de dados'},indent=4)
				status = 404
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
			
		if request.url == 'https://localhost:5000/musicas/all/':#DELETE ALL MUSICAS 
			try:
		...
		if request.url == 'https://localhost:5000/musicas/all/artista/' + str(id): #DELETE ALL MUSICAS_A <id_artista> 
			dados=json.loads(request.data)
			informacao = (int(dados['id_artista']),)
			try:
		...
		if request.url == 'https://localhost:5000/musicas/all/utilizador/' + str(id): #DELETE ALL MUSICAS_U <id_user> 
			dados=json.loads(request.data)
			informacao = (int(dados['id_user']),)
			try:
		...
		if request.url == 'https://localhost:5000/musicas/all/avaliacao': #DELETE ALL MUSICAS <avaliacao> 
			dados = json.loads(request.data)
			informacao = (dados['avaliacao'],)
			try:
...
def artistas(id = None):
	if request.method == "GET":
		if request.url == 'https://localhost:5000/artistas/' + str(id): #READ ARTISTA <id_artista> 
			dados=json.loads(request.data)
			informacao = (dados['id_artista'],)
			try:
				cur_id_spotify = g.db[1].execute('SELECT id_spotify FROM artistas WHERE id = ?', informacao) 
			
				cur_id_spotify = cur_id_spotify.fetchone()

				if cur_id_spotify is not None:
					id_spotify = cur_id_spotify[0]
					json_spotify = requestArtista(id_spotify)
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas/'+str(id),'httpStatus': 404, 'title' : "Nao foram encontradas informacoes. Id do artista invalido!'"},indent=4)
					status = 404
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas/'+str(id),'httpStatus': 500, 'title' : "Ocorreu um erro na execucao da query"},indent=4)
				status = 500
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
		if request.url == 'https://localhost:5000/artistas/all/': #READ ALL ARTISTAS  
			try:
				cur = g.db[1].execute("SELECT * FROM artistas")
			except sqlite3.OperationalError:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas/all/','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
				status = 500
			else:
				linha = cur.fetchall()
				if linha is not None and len(linha) != 0:
					...
					for info in linha:
						...
						id_spotify = info[1]
						nome = info[2]
						resposta_artista  = requestArtista(str(id_spotify))
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas/all/','httpStatus': 404, 'title' : 'Nao existem artistas na base de dados atualmente'},indent=4)
					status = 404
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
			
	if request.method == "PUT":
		if request.url == 'https://localhost:5000/artistas': #CREATE ARTISTA <id_spotify> 
			dados = json.loads(request.data)
			resposta_artista = requestArtista(dados['id_spotify'])
			if resposta_artista is None:
				resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas','httpStatus': 404, 'title' : 'O id do spotify eh invalido'},indent=4)
				status = 404
				r = make_response((resposta, status, {'Content-Type': 'application/json'}))
				return r
			if "error" not in resposta_artista:
				nome_artista = getNomeFromArtista(resposta_artista)
				...
				if existe_artista is None:
					try:
						...
						resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas','httpStatus': 500, 'title' : 'Ocorreu um erro na execucao da query'},indent=4)
						status = 500
					else:
					...
					resposta = json.dumps({'describedBy' : 'https://localhost:5000/artistas','httpStatus': 409, 'title' : "Ja existe um artista com nome= " + str(nome_artista) +  " e id= " + str(dados['id_spotify']) + " inserido na base de dados"},indent=4)
					status = 409
			else:
				resposta = resposta_artista
				status = 401
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
	...
	if request.method == "DELETE":
		if request.url == 'https://localhost:5000/artistas/' + str(id): #DELETE ARTISTA <id_artista> 
			try:
			...
			r = make_response((resposta, status, {'Content-Type': 'application/json'}))
			return r
		if request.url == 'https://localhost:5000/artistas/all/':#DELETE ALL ARTISTAS 
			try:
